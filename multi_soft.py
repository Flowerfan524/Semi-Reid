...
def multi_soft(configs, data, iter_step=1, gamma=0.3, train_ratio=0.2):
    """
    ...
    for step in range(start_step, iter_step):
        for view in range(num_view):
            ...
            weights[view] = get_weights(
                pred_probs[view], pred_y, train_data, add_ratio, gamma)
            ...
            p_b = mu.predict_prob(model, data.trainval,
...
multi_soft([config1, config2, config3], data, 4)
