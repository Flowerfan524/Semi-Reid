...
def test_acc(model, data, data_dir, config):
    ...
    p_b = mu.predict_prob(model, data, data_dir, config)
...
def spaco(configs,
    ...
    save_dir = os.path.join('logs', 'spaco', regularizer,
                            'seed_%d' % args.seed)
    ###########
    ...
    for view in range(num_view):
        sel_id, weight = dp.get_ids_weights(pred_probs[view], pred_y,
                                            train_data, add_ratio, gamma,
                                            regularizer, num_view)
...
config2 = Config(
    ...
    width=224)
config3 = Config(
    model_name='resnet101', loss_name='weight_softmax', img_translation=2)
